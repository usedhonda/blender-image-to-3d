...
The original is immutable and remains the identity authority. Record the original's provenance separately from the user-adopted production/comparison revision. A cleaned candidate does not silently replace the original, but when the user adopts it as the modeling target, compare production against that exact revision; do not omit it or substitute the original without agreement. Before any production, make a design brief from observed, inferred, and undecided facts. Use GPT-6 Astra at high reasoning for planning and image/design review on an explicit host when available, then use GPT-6 Astra at low reasoning for production on an explicit host with no fork, only after the complete design revision is approved. Never claim this skill changed the model. If a manual model switch is needed, confirm the configured model/host first. No Plan mode is required. High design followed by Low production is the chosen workflow, not a measured token-saving claim.
...
## In-process quality decisions

Judge each meaningful construction result against the adopted target, not merely the previous attempt. Coarse work may omit detail, but must preserve the identity-bearing proportions, expression and silhouette. Technical validity, more vertices, model effort level and local improvement do not establish visual acceptance.

If a result is visibly off-target, classify it as failed before requesting human acceptance. Before another paid/generative/model/render step, name the observed mismatch, its supported cause, the specific intervention and the expected target-relative improvement. If that link is unsupported, stop that repair route and reassess the method; do not spend further calls polishing it or defer structural failure to finishing. This stops an unsupported approach, not the user's goal. Read `references/diagnostics-and-style.md` for the failure patterns and record the decision in `templates/quality.md`.

Human approval decides preference among technically and visually credible candidates; it is not a substitute for the agent rejecting known failures. Failed work may be shown explicitly for diagnosis, never presented as ready for the next stage. A functioning workflow/CLI is not proof of a production-capable modeling recipe: label methods unverified until a target-matching artifact demonstrates them.
