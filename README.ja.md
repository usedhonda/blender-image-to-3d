...
完成例では、起点となる1枚、後から加えた参照と用途、承認設計、粗形状、代表材質、修正前後、完成モデル、書出し再読込の証拠を順番に残します。[制作履歴テンプレート](skills/blender-image-to-3d/templates/production-story.md)を使い、実作業の記録と完成後の演出画像を区別します。非公開の原画を自動で公開することはありません。

[実制作記事の知見](skills/blender-image-to-3d/references/practitioner-lessons.md)は、経路選択・代表部品・再利用可能な品質検査に整理しています。VRoidや素体利用は選択肢であり、必須依存やキャラクター品質の保証ではありません。
